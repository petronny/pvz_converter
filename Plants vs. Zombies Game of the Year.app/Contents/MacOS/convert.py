...
class Userdata:
    ...
    def __init__(self, filename, _format=None):
        ...
        self.tail = _bytes[-0x44:]
        self.plants = self.load_plant(_bytes)
    ...
    def guess_format(self, _bytes):
        if len(_bytes[0x334 + self.num_plant * 0x58 :]) == 0x44:
            return 'windows'
        if len(_bytes[0x334 + self.num_plant * 0x3C :]) == 0x44:
            return 'mac'
        raise Exception('Incorrect input format')

    def load_plant(self, _bytes):
        plant_size = 0x58 if self._format == 'windows' else 0x3C
        return [_bytes[0x334 + i * plant_size : 0x334 + (i + 1) * plant_size] for i in range(self.num_plant)]
